...
| ENG-022 | P1 | 已确认 | 配置管理 | 环境配置手工解析且共享 config 包未发挥作用 |
...
- 阶段 0 验收基线：`pnpm ci:check` 通过；Server 62、Web 52、Shared 27、Workspace 15 个测试均通过；14 项数据库 RLS/权限测试通过；数据库迁移从零重建通过；生产镜像构建及容器内应用加载通过。
