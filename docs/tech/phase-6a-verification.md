...
| live Agent runs | canvas-scoped `agent-run-controller.ts` | one WebSocket listener survives chat sidebar remounts; terminal snapshots are acknowledged after UI observation and otherwise bounded to 20 items/30 minutes |

ENG-038 is resolved. The model catalog path consumes shared Query state, and `canvas/page.tsx` now owns one run controller and one underlying WebSocket listener per canvas. Chat sidebar unmount/remount no longer ends live tracking or fallback persistence; reconnect reuses the existing canvas cursor.

## Known warnings and rollback

- The aggregate `pnpm ci:check` remains blocked at Biome by 1 historical error and 506 warnings in pre-existing Agent code. The focused changed-file, type, Web, Server, and workspace gates above pass; warning cleanup remains outside this ownership change.
