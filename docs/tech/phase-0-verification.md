...
本阶段没有把所有长期架构问题一次性解决。Agent 真正隔离执行和 Skill 完整供应链治理已先关闭风险入口，目标方案继续保留在阶段 3。
...
| 自动化测试 | 通过 | Server 62、Web 52、Shared 27、Workspace 15，共 156 项 |
...
- ENG-030：外部 Skill 默认关闭、导入后默认禁用并要求审阅；签名、来源锁定、权限清单和隔离执行仍待阶段 3。
