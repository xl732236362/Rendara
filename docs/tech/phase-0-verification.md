...
本阶段没有把所有长期架构问题一次性解决。Agent 任意执行入口已临时关闭，阶段 3 将永久删除该能力；外部 Skill 风险入口也将在阶段 3 永久删除，改为仓库清单内置 Skill。
...
- ENG-027：生产本地 Shell 默认关闭；阶段 3 将删除 Agent 的 `execute`、LocalShellBackend 和恢复开关，不再建设远端 Sandbox。
