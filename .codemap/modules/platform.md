...
Workspace invariants use Node test; server/shared/web behavior uses Vitest; browser workflows use Playwright. TypeScript-AST architecture enforcement registers initialized and uninitialized variable binding patterns, resolves identifiers from their nearest source/function/block/catch declaration, propagates identity taint through nested bindings, preserves domain and Fastify import provenance by declaration identity under shadowing, evaluates composed V2 URLs, and emits fail-closed mutation diagnostics without aborting later scans. Unified Fastify route discovery feeds both scanner and inventory audit. Structured bounded contracts require the jobs cap, assignments, and consumption to share the owner method body's lexical query declaration while excluding nested function/class bodies; the attachment cap remains tied to the exact repository call's top-level argument property. Shadowed, nested, dynamic, or unrelated same-method decoys fail closed. The GET inventory remains 29 production routes (13 singleton plus 16 collection), with uncapped catalogs and compatibility routes explicit gaps. Verification evidence and removal/rollback windows live in `docs/tech/phase-6a-verification.md`.
