...
export type BrandKitService = {
  ...
  ): Promise<CursorPage<BrandKitSummary>>;
...
export function createBrandKitService(options: {
  ...
  return {
    ...
    async listKitsPage(user, workspaceId, query) {
      ...
      if (error) {
        options.logger?.error("pagination.query_failed", {
      ...
      >();
      for (const asset of assets ?? []) {
        const value = counts.get(asset.kit_id) ?? {
          color: 0,
          font: 0,
          logo: 0,
          image: 0,
      ...
      const items = kits.map(
        (kit): BrandKitSummary => ({
          ...kit,
          asset_counts: counts.get(kit.id) ?? {
            color: 0,
            font: 0,
            logo: 0,
            image: 0,
...
function requireBrandKitCursorCodec(
  codec: CursorCodec | undefined,
): CursorCodec {
  if (!codec)
    throw new Error("CursorCodec is required for paged brand kit queries.");
  return codec;
}

function serializeBrandKitKeyset(boundary: {
  timestamp: string;
  id: string;
}): string {
