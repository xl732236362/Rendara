...
describe("brand kit service cursor pagination", () => {
  it("requests limit + 1, uses created_at/id ascending, trims, and encodes the returned tail", async () => {
    const db = database({
      brand_kits: kits,
      brand_kit_assets: [{ kit_id: kits[0]!.id, asset_type: "color" }],
    });
    const service = createBrandKitService({
  ...
  it("rejects a signed wrong-scope cursor before collection access", async () => {
    ...
    await expect(
      service.listKitsPage(user, workspaceId, { cursor, limit: 2 }),
  ...
  it("applies the decoded ascending tie-break boundary on the next page", async () => {
    ...
    const result = await service.listKitsPage(user, workspaceId, {
      cursor,
      limit: 1,
    });
  ...
  it("fails without a cursor codec before creating a client or accessing the collection", async () => {
    ...
    await expect(
      service.listKitsPage(user, workspaceId, { limit: 1 }),
    ).rejects.toThrow("CursorCodec is required for paged brand kit queries.");
    expect(createUserClient).not.toHaveBeenCalled();
    expect(db.tables).toEqual([]);
  });
});

function kit(id: string, created_at: string) {
  return {
    id,
    name: id,
    is_default: false,
