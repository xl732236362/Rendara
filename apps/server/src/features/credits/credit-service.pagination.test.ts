...
describe("credit service cursor pagination", () => {
  it("uses the authorized scope, requests limit + 1, orders deterministically, and trims", async () => {
    ...
    const result = await service.listTransactionsPage(workspaceId, userId, {
      limit: 2,
    });
  ...
  it("rejects a wrong authorized scope before transaction access", async () => {
    ...
    await expect(
      service.listTransactionsPage(workspaceId, userId, { cursor, limit: 2 }),
  ...
  it("fails without a cursor codec before creating an admin client or accessing transactions", async () => {
    ...
    await expect(
      ...
      "CursorCodec is required for paged credit transaction queries.",
    );
...
function transaction(id: string, created_at: string) {
  return {
    id,
    transaction_type: "generation" as const,
    amount: -1,
    balance_after: 10,
    job_id: null,
    description: null,
