...
describe("project service cursor pagination", () => {
  it("requests limit + 1, orders deterministically, trims, and enriches only the page", async () => {
    const db = database({
      projects: rows,
      canvases: rows.slice(0, 2).map((row) => ({
        id: `canvas-${row.id}`,
        name: "Main",
        is_primary: true,
        project_id: row.id,
  ...
  it("authorizes the workspace before decoding and rejects wrong scope before projects access", async () => {
    ...
    await expect(
      service.listProjectsPage(user, { cursor, limit: 2 }),
  ...
  it("logs a safe enrichment failure when a paged project has no primary canvas", async () => {
    ...
    await expect(
      service.listProjectsPage(user, { limit: 1 }),
  ...
  const client = {
    from(table: string) {
      ...
      query.maybeSingle = async () => ({
        data: table === "workspaces" ? workspace : null,
        error: null,
      });
    ...
    storage: {
      from: () => ({
        getPublicUrl: (path: string) => {
          publicUrlPaths.push(path);
          return { data: { publicUrl: `https://assets/${path}` } };
        },
      }),
    },
