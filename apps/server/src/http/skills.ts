...
const workspaceSkillInstallRequestSchema = z.object({
...
export async function registerSkillRoutes(
  ...
  app.get("/api/skills/:id", async (request, reply) => {
    ...
    const { id } = parseStringParams(request.params, ["id"]);
    const client = options.createUserClient(user.accessToken);
    const { data, error } = await untypedFrom(client, "skills")
      .select("*")
      .eq("id", id)
    ...
    if (!data) {
      return sendSkillError(reply, "skill_not_found", "Skill not found.", 404);
    }
  ...
  app.post("/api/skills/import", async (request, reply) => {
    ...
    if (skillData?.id) {
      await untypedFrom(client, "workspace_skills").upsert(
        createImportedWorkspaceSkillRow(workspaceId, skillData.id, user.id),
  ...
  app.post("/api/workspaces/skills", async (request, reply) => {
    ...
    const body = parseRequest(workspaceSkillInstallRequestSchema, request.body);
  ...
  app.delete("/api/workspaces/skills/:skillId", async (request, reply) => {
    ...
    const { skillId } = parseStringParams(request.params, ["skillId"]);
    const viewer = await options.viewerService.ensureViewer(user);
    const workspaceId = viewer.workspace.id;
    const client = options.createUserClient(user.accessToken);
